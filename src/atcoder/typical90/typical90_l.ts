...
function main(input: string[]) {
    ...
    for (let qx = 0; qx < q; qx++) {
        if (qqn[qx][0] == 1) {
            hw[qqn[qx][1]][qqn[qx][2]] = qx;
        } else {
            if (hw[qqn[qx][1]][qqn[qx][2]] < qx) { console.log("No"); continue; }
            if (hw[qqn[qx][3]][qqn[qx][4]] < qx) { console.log("No"); continue; }
            // sub task
            ...
            while (point = points.pop()) {
                ...
                if (0 < point[0]) {
                    ...
                    if (hw[next[0]][next[1]]) points.push(next);
                }
                if (point[0] < h - 1) {
                    next = [point[0] + 1, point[1]];
                    if (hw[next[0]][next[1]]) points.push(next);
